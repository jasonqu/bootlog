...
import com.github.bootlog.util.Config
...
object Application extends App {
  ...
  def process(config : String) : File = {
    println(config)
    ...
    val ps = pages.map {
      message("production_url") + "/" + _ + "\n"
    }.mkString
    val pts = posts.map {
      message("production_url") + "/post/" + _.name
    }.mkString("\n")
  ...
  process(ConfigFactory.load().root().render())
}
